...
import com.saza.intellical.adapters.QuickFilterEventTypeAdapter
import com.saza.intellical.databases.EventsDatabase
import com.saza.intellical.databinding.ActivityMainBinding
import com.saza.intellical.dialogs.SelectEventTypesDialog
import com.saza.intellical.dialogs.SetRemindersDialog
import com.saza.intellical.extensions.*
import com.saza.intellical.fragments.*
import com.saza.intellical.helpers.*
import com.saza.intellical.helpers.Formatter
import com.saza.intellical.helpers.IcsImporter.ImportResult
import com.saza.intellical.jobs.CalDAVUpdateListener
import com.saza.intellical.models.Event
import com.saza.intellical.models.ListEvent
import com.saza.intellical.models.ListItem
import com.saza.intellical.models.ListSectionDay
...
import com.saza.commons.models.Release
import com.saza.commons.models.SimpleContact
import com.saza.commons.views.MyLinearLayoutManager
...
class MainActivity : SimpleActivity(), RefreshRecyclerViewListener {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Removed appLaunched call to prevent commons library popups
        setupOptionsMenu()
        refreshMenuItems()
        ...
        binding.fabNaturalLanguageLabel.setOnClickListener { openNaturalLanguageEvent() }
        binding.fabNaturalLanguageIcon.setOnClickListener {
            openNaturalLanguageEvent()
    ...
    private fun checkShortcuts() {
        ...
        if (isNougatMR1Plus() && config.lastHandledShortcutColor != appIconColor) {
            ...
            shortcuts.add(getNewNaturalLanguageEventShortcut(appIconColor))
    ...
    @SuppressLint("NewApi")
    private fun getNewNaturalLanguageEventShortcut(appIconColor: Int): ShortcutInfo {
        val newNaturalLanguageEvent = getString(R.string.new_natural_language_event)
        val newNaturalLanguageEventDrawable = resources.getDrawable(R.drawable.shortcut_natural_language_event, theme)
        (newNaturalLanguageEventDrawable as LayerDrawable).findDrawableByLayerId(R.id.shortcut_natural_language_event_background).applyColorFilter(appIconColor)
        val newNaturalLanguageEventBitmap = newNaturalLanguageEventDrawable.convertToBitmap()
        val newNaturalLanguageEventIntent = Intent(this, SplashActivity::class.java)
        newNaturalLanguageEventIntent.action = SHORTCUT_NEW_NATURAL_LANGUAGE_EVENT
        return ShortcutInfo.Builder(this, "new_natural_language_event")
            .setShortLabel(newNaturalLanguageEvent)
            .setLongLabel(newNaturalLanguageEvent)
            .setIcon(Icon.createWithBitmap(newNaturalLanguageEventBitmap))
            .setIntent(newNaturalLanguageEventIntent)
            .build()
    }
    ...
    private fun showExtendedFab() {
        ...
        binding.apply {
            val fabElements = mutableListOf(fabEventLabel, fabExtendedOverlay, fabTaskIcon, fabTaskLabel)

            fabElements.add(fabNaturalLanguageIcon)
            fabElements.add(fabNaturalLanguageLabel)
    ...
    private fun openNaturalLanguageEvent() {
        hideKeyboard()
        val lastFragment = currentFragments.last()
        val currentDayCode = lastFragment.getNewEventDayCode()

        try {
            Intent(applicationContext, NaturalLanguageEventActivity::class.java).apply {
                // Pass the current day context to help with relative date parsing
                putExtra(NEW_EVENT_START_TS, getNewEventTimestampFromCode(currentDayCode))
                startActivity(this)
            }
        } catch (e: Exception) {
            toast(R.string.natural_language_event_error)
        }
    }
    ...
    private fun launchAbout() {
        ...
        if (!resources.getBoolean(R.bool.hide_google_relations)) {
            faqItems.add(FAQItem(R.string.faq_2_title_commons, R.string.faq_2_title_commons))
